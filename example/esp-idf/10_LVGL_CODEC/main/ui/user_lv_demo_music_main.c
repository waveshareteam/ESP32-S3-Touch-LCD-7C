...
/**
 * @brief Audio player callback to handle song end event.
 */
void speaker_callback(audio_player_cb_ctx_t *ctx)
{
    play_state = ctx->audio_event;
    if (play_state == 0) lv_demo_music_album_next(true);
}
...
//Use a timer for updates to prevent system crashes during the update process.
static void ui_update_timer_cb(lv_timer_t * timer) {

    uint32_t id = (uint32_t)lv_timer_get_user_data(timer);  // Retrieve the input parameters
    ...
    else
    {
        ...
        {
            //Stop the song when in the paused state
            speaker_player_play_file(Mp3Path[track_id]);
            audio_player_pause(); // Pause playback
        }
...
# endif
    ...
    LV_IMAGE_DECLARE(user_img_lv_demo_music_logo);
    lv_obj_t * logo = lv_image_create(lv_screen_active());
    lv_image_set_src(logo, &user_img_lv_demo_music_logo);
...
void lv_demo_music_resume(void)
{
    ...
    /************************************************************** */
    printf("\n\nlv_demo_music_resume:%d\r\n\n",play_state);
    switch (play_state)
    {
        case AUDIO_PLAYER_CALLBACK_EVENT_IDLE:
            speaker_player_play_file(Mp3Path[track_id]); // Start playback
            break;
        ...
        case AUDIO_PLAYER_CALLBACK_EVENT_PAUSE:
            audio_player_resume(); // Resume playback
            break;
        default:
            break;
    }
...
static void create_wave_images(lv_obj_t * parent)
{
    LV_IMAGE_DECLARE(user_img_lv_demo_music_wave_top);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_wave_bottom);
    lv_obj_t * wave_top = lv_image_create(parent);
    lv_image_set_src(wave_top, &user_img_lv_demo_music_wave_top);
    ...
    lv_image_set_src(wave_bottom, &user_img_lv_demo_music_wave_bottom);
    ...
    LV_IMAGE_DECLARE(user_img_lv_demo_music_corner_left);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_corner_right);
    lv_obj_t * wave_corner = lv_image_create(parent);
    lv_image_set_src(wave_corner, &user_img_lv_demo_music_corner_left);
    ...
    lv_image_set_src(wave_corner, &user_img_lv_demo_music_corner_right);
#if LV_DEMO_MUSIC_ROUND == 0
...
static void icon_event_cb(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_CLICKED) {
        if(lv_obj_has_flag(ui_Slider1, LV_OBJ_FLAG_HIDDEN)) {
            // If it is hidden, then display it.
            lv_obj_remove_flag(ui_Slider1, LV_OBJ_FLAG_HIDDEN);
        } else {
            // If it is displayed, then hide it.
...
static lv_obj_t * create_icon_box(lv_obj_t * parent)
{
    ...
    lv_obj_t * icon;
    LV_IMAGE_DECLARE(user_img_lv_demo_music_icon_1);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_icon_2);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_icon_3);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_icon_4);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_icon_5);

    icon = lv_image_create(cont);
    lv_image_set_src(icon, &user_img_lv_demo_music_icon_1);
    icon = lv_image_create(cont);
    lv_image_set_src(icon, &user_img_lv_demo_music_icon_2);
    icon = lv_image_create(cont);
    lv_image_set_src(icon, &user_img_lv_demo_music_icon_3);
    icon = lv_image_create(cont);
    lv_image_set_src(icon, &user_img_lv_demo_music_icon_4);

    icon = lv_image_create(cont);
    lv_obj_add_flag(icon, LV_OBJ_FLAG_CLICKABLE);
    lv_image_set_src(icon, &user_img_lv_demo_music_icon_5);
...
static lv_obj_t * create_spectrum_obj(lv_obj_t * parent)
{
    ...
    LV_IMAGE_DECLARE(user_img_lv_demo_music_slider_knob);
    ui_Slider1 = lv_slider_create(obj);

    lv_obj_add_flag(ui_Slider1, LV_OBJ_FLAG_HIDDEN);

    lv_slider_set_value(ui_Slider1, 50, LV_ANIM_OFF);
    if(lv_slider_get_mode(ui_Slider1) == LV_SLIDER_MODE_RANGE) lv_slider_set_left_value(ui_Slider1, 0, LV_ANIM_OFF);
    lv_obj_set_width(ui_Slider1, 3);
    lv_obj_set_height(ui_Slider1, 150);
    lv_obj_set_x(ui_Slider1, 270);
    lv_obj_set_y(ui_Slider1, -40);
    lv_obj_set_align(ui_Slider1, LV_ALIGN_CENTER);

    lv_obj_set_style_bg_image_src(ui_Slider1, &user_img_lv_demo_music_slider_knob, LV_PART_KNOB);
    lv_obj_set_style_bg_opa(ui_Slider1, LV_OPA_TRANSP, LV_PART_KNOB);
    lv_obj_set_style_pad_all(ui_Slider1, 20, LV_PART_KNOB);
    lv_obj_set_style_bg_grad_dir(ui_Slider1, LV_GRAD_DIR_HOR, LV_PART_INDICATOR);
    lv_obj_set_style_bg_color(ui_Slider1, lv_color_hex(0x569af8), LV_PART_INDICATOR);
    lv_obj_set_style_bg_grad_color(ui_Slider1, lv_color_hex(0xa666f1), LV_PART_INDICATOR);
    lv_obj_set_style_outline_width(ui_Slider1, 0, 0);
...
static lv_obj_t * create_ctrl_box(lv_obj_t * parent)
{
    ...
    LV_IMAGE_DECLARE(user_img_lv_demo_music_btn_loop);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_btn_rnd);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_btn_next);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_btn_prev);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_btn_play);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_btn_pause);

    lv_obj_t * icon;
    icon = lv_image_create(cont);
    lv_image_set_src(icon, &user_img_lv_demo_music_btn_rnd);
    lv_obj_set_grid_cell(icon, LV_GRID_ALIGN_START, 1, 1, LV_GRID_ALIGN_CENTER, 0, 1);

    icon = lv_image_create(cont);
    lv_image_set_src(icon, &user_img_lv_demo_music_btn_loop);
    lv_obj_set_grid_cell(icon, LV_GRID_ALIGN_END, 5, 1, LV_GRID_ALIGN_CENTER, 0, 1);

    icon = lv_image_create(cont);
    lv_image_set_src(icon, &user_img_lv_demo_music_btn_prev);
    ...
    lv_imagebutton_set_src(play_obj, LV_IMAGEBUTTON_STATE_RELEASED, NULL, &user_img_lv_demo_music_btn_play, NULL);
    lv_imagebutton_set_src(play_obj, LV_IMAGEBUTTON_STATE_CHECKED_RELEASED, NULL, &user_img_lv_demo_music_btn_pause, NULL);
    ...
    lv_obj_set_width(play_obj, user_img_lv_demo_music_btn_play.header.w);

    icon = lv_image_create(cont);
    lv_image_set_src(icon, &user_img_lv_demo_music_btn_next);
    ...
    lv_obj_set_style_bg_image_src(slider_obj, &user_img_lv_demo_music_slider_knob, LV_PART_KNOB);
...
void ui_event_Slider1(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_LONG_PRESSED_REPEAT || event_code == LV_EVENT_CLICKED) {
        int16_t value = lv_slider_get_value(ui_Slider1);
        speaker_codec_volume_set(value,NULL);
    }
}
...
static void track_load(uint32_t id)
{
    ...
    lv_timer_t * t = lv_timer_create(ui_update_timer_cb, 0, (void *)id); // Cycle can be freely set > 0
    lv_timer_set_repeat_count(t, 1); // Execute only once
}
...
static lv_obj_t * album_image_create(lv_obj_t * parent)
{
    LV_IMAGE_DECLARE(user_img_lv_demo_music_cover_1);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_cover_2);
    LV_IMAGE_DECLARE(user_img_lv_demo_music_cover_3);
    ...
    switch(track_id) {
        case 2:
            lv_image_set_src(img, &user_img_lv_demo_music_cover_3);
            spectrum = BGM_2;
            spectrum_len = sizeof(BGM_2) / sizeof(BGM_2[0]);
            break;
        case 1:
            lv_image_set_src(img, &user_img_lv_demo_music_cover_2);
            spectrum = BGM_1;
            spectrum_len = sizeof(BGM_1) / sizeof(BGM_1[0]);
            break;
        case 0:
            lv_image_set_src(img, &user_img_lv_demo_music_cover_1);
            spectrum = For_Elise;
            spectrum_len = sizeof(For_Elise) / sizeof(For_Elise[0]);
            break;
    }
